...
class WoTracingList extends StatefulWidget {
  ...
  const WoTracingList({Key? key}) : super(key: key);
  @override
...
class _WoTracingListState extends State<WoTracingList> {
    ...
    return WillPopScope(
      ...
      child: Scaffold(
          appBar: AppBar(
            ...
            leading: IconButton(
                onPressed: () {
                  Navigator.push(context, MaterialPageRoute(builder: (context) => MyHomePage())); //   Navigator.of(context).pop();
                },
          ...
          body: Stack(
            children: [
                ...
                children: [
                  workOrderProvider.tracingListView.isNotEmpty
                      ? Expanded(
                          ...
                          onNotification: workOrderProvider.notificationController,
                          child: ListView.builder(
                              itemCount: workOrderProvider.tracingListView.length,
                              itemBuilder: (BuildContext context, int i) {
                                l++;
                                if (l == 5) {
                                  l = 0;
                                }
                                TracingViewModal listElements = workOrderProvider.tracingListView[i];
                                        ...
                                        : Navigator.push(
                                            ...
                                            MaterialPageRoute(
                                              builder: (context) => WoList(
                                                pageController: mainViewProvider.pageController!,
              ...
              if (workOrderProvider.isDataLoading == true) ...[loadingBar(context, APPColors.Accent.grey, APPColors.Main.black)],
            ],
          )),
    );
  }
  ...
  Widget sayfaYenile() {
    return Consumer<WorkOrderProvider>(builder: (context, WorkOrderProvider, child) {
      return InkWell(
        onTap: () {
          setState(() {
